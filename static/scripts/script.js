...
document.addEventListener("click", function (e) {
    ...
    if (e.target.id === "confirmTablesBtn") {
        ...
        // Sync to local table.csv + GitHub
        fetch("/save-tables", {
            method: "POST",
            ...
            .then(result => {
                if (result.success) {
                    console.log("✅ table.csv saved locally and pushed to GitHub:", result.github);
                } else {
                    console.warn("⚠️ Save error:", result.error);
                }
            })
            .catch(err => console.warn("⚠️ Could not reach save server:", err));
